...
def local_date_of(timestamp: str | None) -> str | None:
    """완료시각 문자열을 '로컬 기준 날짜(YYYY-MM-DD)'로 바꾼다.

    저장소가 서로 다른 모양을 준다:
      SQLite   '2026-09-07 16:33:12'        시간대 없음 · 이미 로컬시각
      Postgres '2026-09-07T07:33:12+00:00'  시간대 붙은 UTC

    앞 10글자를 자르면 UTC 쪽이 하루 어긋난다 — UTC 23시는 서울에서 이미 다음 날이다.
    읽을 수 없는 값은 예외 대신 None을 준다. 요약 한 줄 때문에 명령이 죽으면 안 된다.
    """
    if not timestamp:
        return None
    value = str(timestamp).strip()
    if value.endswith(("Z", "z")):
        value = value[:-1] + "+00:00"  # fromisoformat은 'Z'를 3.11 미만에서 못 읽는다
    try:
        moment = datetime.fromisoformat(value)
    except ValueError:
        return None
    if moment.tzinfo is not None:
        moment = moment.astimezone()  # 시스템 로컬 시간대로 옮긴 뒤 날짜를 뽑는다
