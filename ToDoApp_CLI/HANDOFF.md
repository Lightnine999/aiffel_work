...
> · 웹 화면 색상 팔레트 변경(§17) · Supabase 이행 작업은 보류
...
## 17. 웹 화면 색상 팔레트 (2026-09-21)

"무채색 느낌이 아쉽다"는 피드백으로 `base.html`의 CSS 변수만 바꿨다. 구조는 그대로다
(같은 카드·탭·인풋 레이아웃) — 색을 **할 일 앱이 실제로 갖고 있는 정보(우선순위·완료
여부)에 연결**하는 방향으로 바꿨다. 장식이 아니라 화면을 더 빨리 훑어볼 수 있게 하는
쪽을 택했다.

| 무엇 | 색 | 왜 |
|---|---|---|
| 배경·본문 | 세이지 톤 오프화이트 / 잉크 그린 블랙 | 순수 회색 대신 은은한 녹색 기미 |
| 브랜드(추가 버튼·활성 탭·포커스링) | 소나무 틸(`--accent`, 라이트 `#2f6f62`) | 차분하지만 무채색은 아닌 "실행" 색 |
| 우선순위 **높음** | 클레이/러스트(`--high`) | 목록 왼쪽 스트라이프 3px — 훑어볼 때 바로 걸린다 |
| 우선순위 **낮음** · 완료 상태 | 세이지 그린(`--low`) | "낮음"과 "이미 끝남"을 같은 "신경 안 써도 됨" 계열로 묶었다 |
| 태그 칩 | 브랜드색 옅은 틴트 배경 | 기존 회색 테두리 아웃라인 → 틴트 배경으로, 태그가 눈에 띄게 |
| 오늘 완료 카드 | 세이지 틴트 배경 | 흰 카드 사이에서 "오늘 해냈다"가 바로 구분되게 |
| 기한 지남 · 경고 | 기존 빨강 계열 유지, 톤만 팔레트에 맞춤 | 위험/실수 신호는 바꾸지 않는 게 안전하다 |

라이트/다크 모드 둘 다 같은 토큰 이름으로 값만 뒤집는다(`prefers-color-scheme`).
`--high`/`--low`가 "보통(2)"에는 없다 — 우선순위 3단계를 전부 색칠하면 시끄러워서,
예외(높음·낮음)만 칠하고 기본값은 색 없이 지나간다.

완료 토글 버튼도 `[x]`/`[ ]` 텍스트에서 `✓`/`○` 문자로 바꿨다. 테스트가 그 텍스트에
의존하지 않는 걸 확인하고 바꿨다(`grep`으로 확인).

### 확인한 것

- 전체 462개 테스트 통과 (색상은 자동화 테스트 대상이 아니다 — CSS 값 자체를 검증하는
  테스트는 만들지 않았다. 시각적 변경이라 브라우저로 직접 확인하는 쪽이 맞다)
- 임시 SQLite로 `python3 app.py` 띄워 놓고 실제 브라우저로 확인:
  라이트·다크·모바일(375px) 3가지 모드에서 우선순위 스트라이프·오늘 완료 카드·
  태그 칩·기한 지남 빨강이 의도대로 보이는지 스크린샷으로 점검. 실 데이터베이스는
  건드리지 않았다.

### 남은 것

모바일에서 카드 `box-shadow`가 다소 밋밋해 보일 수 있다 — 필요하면 나중에 조정.
